...
const int S1_pzem1234=27;const int S0_pzem1234=28;

Hshopvn_Pzem004t_V2 pzem5678(&Serial1);
const int S1_pzem5678=29;const int S0_pzem5678=39;
...
void setup(void)
  { 
    ...
    lcd.setCursor(0, 0);lcd.print("POWER CONTROL");
    lcd.setCursor(0, 1);lcd.print("ARB PC-04");
    delay(1000);
...
////////////////////////////////////////////////////  
    ...
    */    
    ...
    pinMode(S1_pzem1234,OUTPUT); pinMode(S0_pzem1234,OUTPUT);
    pzem1234.begin();
    pzem1234.setTimeout(100);
    //digitalWrite(S1_pzem1234,0);digitalWrite(S0_pzem1234,0);pzem1234.resetEnergy();

    pinMode(S1_pzem5678,OUTPUT); pinMode(S0_pzem5678,OUTPUT);
    pzem5678.begin();
...
void doccauhinh()
  {
    ...
    Serial.print("NAME: ");for(int i=0;i<5;i++){Serial.print(chuChar[Name[i]]);}Serial.println("");
    for(int i=0;i<5;i++)
        {
    ...
    mac[4]=ID>>8;mac[5]=ID&0x00FF;//dua vao ID de lay dia chi Mac cho board
    ...
    for(int i=0;i<32;i++)
        {
          if(ServerAddress[i]==0)i=32;
          else serverString += chuString[ServerAddress[i]];  
        }
    serverString.toCharArray(serverChar,serverString.length()+1);
    mqtt_server=serverChar;
    Serial.print("SERVER ADDRESS: ");Serial.println(mqtt_server);
    //////////////////////////////////////////////////////PORT
    port = (portH << 8) + (portL);//Serial.print("port: ");Serial.println(port);
    mqtt_port = port;Serial.print("mqtt_port: ");Serial.println(mqtt_port);
    ...
    Serial.print("mqtt_topic_sub_CONTROL: ");Serial.println(mqtt_topic_sub_CONTROL);

    STATUSString=CLIENT_IDString;
    STATUSString +="/STATUS";
    STATUSString.toCharArray(STATUSChar,STATUSString.length()+1);
    mqtt_topic_sub_STATUS=STATUSChar;
    Serial.print("mqtt_topic_sub_STATUS: ");Serial.println(mqtt_topic_sub_STATUS);
...
//char* mqtt_topic_pub_MODE= "POWER/02/MODE"
    ...
    Serial.print("mqtt_topic_sub_MODE: ");Serial.println(mqtt_topic_pub_MODE);
...
//char* mqtt_topic_pub_RELAY= "POWER/02/RELAY"
    ...
    Serial.print("mqtt_topic_sub_RELAY: ");Serial.println(mqtt_topic_pub_RELAY);
...
//char* mqtt_topic_pub_V= "POWER/02/V"
    ...
    Serial.print("mqtt_topic_sub_V: ");Serial.println(mqtt_topic_pub_V);
...
//char* mqtt_topic_pub_A= "POWER/02/A"
    ...
    Serial.print("mqtt_topic_sub_A: ");Serial.println(mqtt_topic_pub_A);
...
//char* mqtt_topic_pub_TEM= "POWER/02/TEM"
    ...
    Serial.print("mqtt_topic_sub_TEM: ");Serial.println(mqtt_topic_pub_TEM);
...
//char* mqtt_topic_pub_HUM="POWER/02/HUM"
    ...
    Serial.print("mqtt_topic_sub_HUM: ");Serial.println(mqtt_topic_pub_HUM);
     ...
     if (now - timer_ketnoiserver >= timerketnoiserver)//
       {
          ...
          if (mqttClient.connect(CLIENT_ID,mqtt_user,mqtt_pwd, mqtt_topic_sub_STATUS, 1, 1,"0"))//;mqttClient.connect(CLIENT_ID,mqtt_user, mqtt_pwd)) // Thực hiện kết nối với mqtt user và pass
    ...
    
            {
              ...
              lcd.backlight();
              ketnoiserver=1;guitrangthaikhoidong=1;khoidong=1;
            } 
          else 
            {
...
void docenthernet(char* topic, byte* payload, unsigned int length)
   {
      ...
      Serial.println();
      char crcdata[7];
      vitrirelay=int(payload[0]);crcdata[0]=vitrirelay;Serial.print("vitrirelay:");Serial.print(vitrirelay);Serial.print(" ");
      tatmo=int(payload[1]);crcdata[1]=tatmo;Serial.print("tatmo:");Serial.print(tatmo);Serial.print(" ");
      CRCLo=int(payload[2]);     Serial.print("CRCLo:");Serial.print(CRCLo);Serial.print(" ");
      CRCHi=int(payload[3]);    Serial.print("CRCHi:");Serial.println(CRCHi);   
      ...
      if ( (vitrirelay<=20) && (tatmo==0||tatmo==255) &&(CRCLo==(codeCRC >> 8)) && (CRCHi==(codeCRC & 0x00FF)) )
         { 
           ...
           if(tatmo==255){tatmo=1;}else {tatmo=0;}
           lenh=1;
         }
      else
         {
...
void dieukhienrelay()
   {
      //Serial.println("4");
       if (vitrirelay!=0 && vitrirelay<=sorelay){digitalWrite(Relay[vitrirelay-1],not(tatmo));} 
      ...
      if (now - timer_docdienapdongdien> 10000) 
        {
          ...
          for (int i=0; i<9;i++)
            {
              if(i==0){pzemData = pzemTong.getData();} 
              else if(i>=1 && i<=4 )
                {
                  if(i==1)     {digitalWrite(S1_pzem1234,0);digitalWrite(S0_pzem1234,0);}
                  else if(i==2){digitalWrite(S1_pzem1234,0);digitalWrite(S0_pzem1234,1);}
                  else if(i==3){digitalWrite(S1_pzem1234,1);digitalWrite(S0_pzem1234,0);}
                  else if(i==4){digitalWrite(S1_pzem1234,1);digitalWrite(S0_pzem1234,1);}
                  pzemData = pzem1234.getData();
                }
              else if(i>=5 && i<=8 )
                {
                  if(i==5)     {digitalWrite(S1_pzem5678,0);digitalWrite(S0_pzem5678,0);}
                  else if(i==6){digitalWrite(S1_pzem5678,0);digitalWrite(S0_pzem5678,1);}
                  else if(i==7){digitalWrite(S1_pzem5678,1);digitalWrite(S0_pzem5678,0);}
                  else if(i==8){digitalWrite(S1_pzem5678,1);digitalWrite(S0_pzem5678,1);}
                  pzemData = pzem5678.getData();
                }
                                 
              //pzem_info pzemData = pzemTong.getData();
              if((pzemData.volt>=0)&&(pzemData.volt<260)){ dienap[i] = pzemData.volt;}
              if(pzemData.ampe>=0){dongdien[i] = pzemData.ampe;}
              if(pzemData.power>=0){congsuat[i] = pzemData.power;}
              if(pzemData.energy>=0){diennangtieuthu[i] = pzemData.energy;}
...
void hienLCD()
    {
      ...
      if(dongdien[0]>=10){sprintf(temp,"%3dV %3dA",dienap[0],int(dongdien[0]));}
      else{sprintf(temp,"%3dV  %3d.%dA",dienap[0],int(dongdien[0]),dongdienle);}
      lcd.setCursor(0, 0);lcd.print(temp);

      lcd.setCursor(0, 1);
      lcd.print(temp_c1);
      lcd.write(223);lcd.print("C");if(temp_c1<10)lcd.print("   ");else if(temp_c1<100)lcd.print("  ");else lcd.print(" ");
      lcd.setCursor(6, 1);
      lcd.print(humidity1);
      lcd.print("%");if(humidity1<10)lcd.print("   ");else if(humidity1<100)lcd.print("  ");else lcd.print(" ");
      
      lcd.setCursor(12, 1);
      long now = millis();
      if (now - timer_lcd>500) {timer_lcd = now;chupnhay=not chupnhay;}
      if ( chupnhay==0)
        {
          if (digitalRead(Auto)==0){lcd.print("MAN ");}
          else{lcd.print("AUTO");}
        }
      else lcd.print("    ");   
    }
      ...
      for (int i=0;i<8;i++) 
          {
            ...
            if (n[i] != n_1[i]){timer_docdienapdongdien = (millis()-7500);}
            //Serial.print(n[i]);
          } 
          //Serial.println("");
      docdienapdongdien();  
...
void guitrangthai()
   {
      ...
      if(( mode !=  mode1 )|| guitrangthaikhoidong==1)
        {
          ...
          byte modegui=0x00;
          if(mode==0)modegui=0x00;else modegui=0xFF;
          char crcdatagui[1];
      ...
      for (int a=0;a<20;a++) 
          {
            //if (a==0)Serial.println("9.9");
            if (n[a] != n_1[a]){n_1[a]=n[a];guitraithairelay=1; }  
          }
      if( guitraithairelay==1 || guitrangthaikhoidong==1)
        {
          //for (int i=0;i<20;i++) {Serial.print(n[i]); Serial.print(" ");}Serial.println(" ");

          int byte1_n=0;int byte2_n=0;int byte3_n=0;  
          int b=7;
          for (int i=0;i<=7;i++) {byte1_n = byte1_n | n[b]  << i;      byte2_n = byte2_n | n[b+8]  << i;        byte3_n = byte3_n | n[b+16]  << i;b--;}
          char crcdatagui[3];
          crcdatagui[0] = byte1_n;    crcdatagui[1] = byte2_n ;    crcdatagui[2] = byte3_n ;
          unsigned int codeCRCgui = calcCRC16(3, crcdatagui);
          int codeCRCguiLo = codeCRCgui >> 8;
          int codeCRCguiHi = codeCRCgui & 0x00FF;
          
          byte datasend[]={byte1_n,byte2_n,byte3_n,codeCRCguiLo,codeCRCguiHi};
          mqttClient.publish(mqtt_topic_pub_RELAY,datasend,5,1);
          guitraithairelay=0; 
        }
      //////////////////////////////////////////////////////////////DIEN AP
      if( dienap[0] != dienapgui[0] || guitrangthaikhoidong==1)
        {
          ...
          char crcdatagui[2];
          crcdatagui[0] = dienap[0] >>8 ;crcdatagui[1] = dienap[0] &0x00FF ;  
...
void docaserial()
  {
    if(Serial.available()>0)
          { 
                vitrirelay = Serial.parseInt();tatmo = Serial.parseInt();Serial.read();
                lenh=1;Serial.println("Serial");
          }
  }
    ...
    if(Setup_1>0 && Setup_2==0 && Setup_3==0 && Setup_4==0)//lop 1
       {
            if(nhanSetup==0)
              { 
                 if(giatriEncoder<=0){giatriEncoder=5;dongchop=1;}
                 else if(giatriEncoder>5){giatriEncoder=1;dongchop=0;}
                 Setup_1=giatriEncoder;
              }
            else if(nhanSetup==1)
             {
                 lcd.clear();
                 if(Setup_1==1){Setup_2=1;}//vaoxem thong so
                 else if(Setup_1==2){Setup_2=1;}
                 else if(Setup_1==3){Setup_2=1;}//vao setup
                 else if(Setup_1==4){Setup_2=1;}//vao cai dat ngon ngu
                 else if(Setup_1==5){Setup_1=0;}// thoat
                 dongchop=0;giatriEncoder=1;
                 nhanSetup=2;
             }
           choptat();if (chopchu>0){SetupLCD_0();}
       }
//////////////////////////////////////////
            ...
            if(Setup_1==1)//thong so ouput
                {
                     if(nhanSetup==0)
                        {
                          if(giatriEncoder<=0){giatriEncoder=11;}
                          else if(giatriEncoder>=12){giatriEncoder=1;}
                     ...
                     else if(nhanSetup==1)
                        {
                          ...
                          else if(Setup_2==11)Setup_2=0;//thoat
                    ...
                    if(nhanSetup==0)
                        {
                          if(IPtinh==0)
                            {
                              if(giatriEncoder<=0){giatriEncoder=8;}
                              else if(giatriEncoder>8){giatriEncoder=1;} 
                            }
                          else
                            {
                              if(giatriEncoder<=0){giatriEncoder=11;}
                              else if(giatriEncoder>11){giatriEncoder=1;} 
                            }
                          Setup_2=giatriEncoder;
                        }
                     else if(nhanSetup==1)
             ...
             else if(Setup_1==3)//setup
                  {
                      if(nhanSetup==0)
                        {
                          if(giatriEncoder<=0){giatriEncoder=5;dongchop=1;}
                          else if(giatriEncoder>=6){giatriEncoder=1;dongchop=0;}
                          Setup_2=giatriEncoder;
                        }
                      else if(nhanSetup==1)
                        {
                          ...
                          else if(Setup_2==5){Setup_2=0;giatriEncoder=Setup_1;}// Thoat
                          
                          nhanSetup=2;
                        }
                      choptat();if (chopchu>0){SetupLCD_3();}
                   }
              else if(Setup_1==4)//ngon ngu
                    {
                      choptat();if (chopchu>0){SetupLCD_4();}
                      if(nhanSetup==1)
                        {
                          if(dongchop==0){ngonngu=0;}
                          else {ngonngu=1;}
                          Setup_2=0;
                          if  ( ngonngu != EEPROM.read(100) ){ EEPROM.write(100, ngonngu);}
                          nhanSetup=2;
                        }
                    }
        }
       ...
       {            
           if(Setup_1==1 && Setup_2==10)//reset out put
                  {              
                      ...
                      if(nhanSetup==0)  
                        {
                            if(giatriEncoder<=0){giatriEncoder=11;dongchop=1;}
                            else if(giatriEncoder>=12){giatriEncoder=1;}
                      ...
                      else if(nhanSetup==1)
                        {
                            if(Setup_3==1)
                                { 
                                    giatriEncoder=10;Serial.println("reset het");
                                    pzemTong.resetEnergy();
                                    for (int i=0; i<9;i++)
                                       {
                                            if(i==0){pzemTong.resetEnergy();} 
                                            else if(i>=1 && i<=4 )
                                              {
                                                if(i==1)     {digitalWrite(S1_pzem1234,0);digitalWrite(S0_pzem1234,0);}
                                                else if(i==2){digitalWrite(S1_pzem1234,0);digitalWrite(S0_pzem1234,1);}
                            ...
                            else if(Setup_3==11){giatriEncoder=10;}
                            else
                              {
           ...
           else if(Setup_1==3 && Setup_2==1)// vao setup NAME
                  {
                      if(nhanSetup==0)
                        {                     
                          if(setchu==0)//khongsetchu
                            {
                              if(giatriEncoder<=0){giatriEncoder=6;}
                              else if(giatriEncoder>=7){giatriEncoder=1;}
                              Setup_3=giatriEncoder;
                            }
                          else//davaosetchu
                            {
                              if(giatriEncoder<0){giatriEncoder=62;}
                              else if(giatriEncoder>62){giatriEncoder=0;}
                      ...
                      else if(nhanSetup==1)
                        {
                          if(Setup_3==6)//thoat
                            { 
                              Setup_3=0;dongchop=0;
                              for(int i=0;i<5;i++){if(Nametemp[i] != Name[i])resetBoard();} 
                            }
                          else//setchu
                            {
                              if(setchu==0){setchu=1;giatriEncoder=Name[Setup_3-1];}//vaosetchu
                              else//thoatsetchu
                                {
                                  if ( Name[Setup_3-1] != EEPROM.read(100+Setup_3) ){ EEPROM.write(100+Setup_3, Name[Setup_3-1]);} 
                                  setchu=0;Setup_3 = Setup_3 + 1;giatriEncoder=Setup_3;
                                }
                      ...
                      if(Setup_3==6){dongchop=1;choptat();if (chopchu>0){SetupLCD_3_1();}}
                      else SetupLCD_3_1();   
                  }
                  ...
                  {    
                      if(nhanSetup==0)  
                        {
                            if(giatriEncoder<=0){giatriEncoder=999;}
                            else if(giatriEncoder>=1000){giatriEncoder=1;}
                            IDtam=giatriEncoder;
                        }
                      else if(nhanSetup==1)
                        {
                            if  ( giatriEncoder != EEPROM.read(106) ){ID = giatriEncoder; EEPROM.write(106,ID);delay(100);resetBoard();}
                            Setup_3=0;
                            nhanSetup=2;
                        }
                      SetupLCD_3_2();
           ...
           else if(Setup_1==3 && Setup_2==3)//IP SETUP
                  {
                      if(nhanSetup==0)  
                        {
                           if(giatriEncoder<=0){giatriEncoder=2;}
                           else if(giatriEncoder>=3){giatriEncoder=1;}
                           Setup_3=giatriEncoder;
                        }
                      else if(nhanSetup==1)
                        {
                            ...
                            if(Setup_3==1)//lay ip dong
                               {
                                 if ( IPtinh != EEPROM.read(120) ){ EEPROM.write(120, IPtinh);resetBoard();}
                                 //Setup_3=0;
                               }
                            else//lay ip tinh
                               {
                                  Setup_4=1;dongchop=0;
                                  giatriEncoder=1; 
                                  setgiatriIP=256;    
                               }
                            nhanSetup=2;
           ...
           else if(Setup_1==3 && Setup_2==4)//SERVER SETUP
                  {
                      if(nhanSetup==0)  
                      ...
                      else if(nhanSetup==1)
                        {
                            if(Setup_3==5)//THOAT
                               {
                                 for(int i=0;i<32;i++){if(ServerAddresstemp[i] != ServerAddress[i])resetBoard();} 
                                 if(port != mqtt_port)resetBoard();
                                 
                                 Setup_3=0;giatriEncoder=Setup_2;
                               }
                            else if(Setup_3==1)//SERVER ADDRESS
                               {
                                 Setup_4=1;giatriEncoder=Setup_4;
                               }
                            else if(Setup_3==2)//PORT
                               {
                                 Setup_4=1;giatriEncoder=port;
                               }
                            nhanSetup=2;
                        }
                      choptat();if (chopchu>0){SetupLCD_3_4();}
                  }
...
/////////////////////////////////////////////////////
    else if(Setup_1>0 && Setup_2>0 && Setup_3>0 && Setup_4>0)//lop 4
       {
          if(Setup_1==3 && Setup_2==3 && Setup_3==2)//vao SETUP IP tinh
                ...
                if(nhanSetup==0)  
                  {
                    if(setIPtinh==0)
                      {
                        if(giatriEncoder<=0){giatriEncoder=4;}
                        else if(giatriEncoder>=5){giatriEncoder=1;}
                    ...
                    else
                      {
                        if (setgiatriIP==256)
                          {
                            if(giatriEncoder<1){giatriEncoder=5;}
                            else if(giatriEncoder>5){giatriEncoder=1;}
                            setIPtinh=giatriEncoder;
                          }
                        else
                          {
                            if(giatriEncoder<0){giatriEncoder=255;}
                            else if(giatriEncoder>255){giatriEncoder=0;}
                  ...
                  {      
                    if(Setup_4==1)//ID ADDRESS
                      {
                        if(setIPtinh==0){setIPtinh=1;giatriEncoder=setIPtinh;}//vao set IP address
                        else
                          {  
                            if(setgiatriIP==256)
                              {   
                                  if(setIPtinh==5)//thoat set IP address
                                    {
                                      setIPtinh=0;giatriEncoder=Setup_4;
                                    }
                                  else
                                    {
                      ...
                      {
                        if(setIPtinh==0){setIPtinh=1;giatriEncoder=setIPtinh;}//vao set DEFAULT GATEWAY
                        else
                          {  
                            if(setgiatriIP==256)
                              {   
                      ...
                      {
                        Setup_4=0;giatriEncoder=2;
                        //kiemtra iP tinh co thay doi reset
                        int reset=0;
                        if  ( IPtinh != EEPROM.read(120) ){EEPROM.write(120, IPtinh);reset=1;}
                        for(int i=0;i<4;i++)
                          { 
                            if  ( ipaddress[i]!=EEPROM.read(121+i)){EEPROM.write(121+i, ipaddress[i]);reset=1;}
                            if  ( gateway[i]!=EEPROM.read(125+i)){EEPROM.write(125+i, gateway[i]);reset=1;}
                            if  ( subnet[i]!=EEPROM.read(129+i)){EEPROM.write(129+i, subnet[i]);reset=1;}
                          } 
                        if(reset==1)resetBoard();              
                      }
                    nhanSetup=2;
                  } 
                SetupLCD_3_3_2();
         ...
         else if(Setup_1==3 && Setup_2==4 && Setup_3==1)//vao SETUP SERVER ADDRESS 
             {
                 ...
                 for(int i=0;i<32;i++){if(ServerAddress[i]==0){vitricokhoangtrang=i+1;i=32;} }
                 ...
                 if(nhanSetup==0)
                      ...
                      if(setchu==0)//khongsetchu
                        { 
                           if(vitricokhoangtrang==0)//khong phat hien khoang trang
                            {
                               if(giatriEncoder<=0){giatriEncoder=33;}
                               else if(giatriEncoder>33){giatriEncoder=1;}
                               Setup_4=giatriEncoder;
                            }
                           else
                            {
                               if(giatriEncoder<=0){giatriEncoder=33;}
                                ...
                                {
                                  if(giatriEncoder>Setup_4)giatriEncoder=33;
                                  else giatriEncoder=vitricokhoangtrang;
                                }
                        ...
                        {
                          if(giatriEncoder<0){giatriEncoder=65;}
                          else if(giatriEncoder>65){giatriEncoder=0;}
                 ...
                 else if(nhanSetup==1)
                   {
                      if(Setup_4==33)//thoat
                        { 
                           Setup_4=0;giatriEncoder = Setup_3;//dongchop=0;                         
                        }
                      else//setchu
                        {
                           if(setchu==0){setchu=1;giatriEncoder=ServerAddress[Setup_4-1];}//vaosetchu
                           else//thoatsetchu
                             {
                               if(ServerAddress[Setup_4-1]==0)//phat hien khoang trang 
                                  {
                                     for(int i=Setup_4-1;i<32;i++){ServerAddress[i]=0;}//xoa tat ca ky tu phia sau
                                     Setup_4 = 32;
                                  }
                               if ( ServerAddress[Setup_4-1] != EEPROM.read(140+Setup_4) ){ EEPROM.write(140+Setup_4, ServerAddress[Setup_4-1]);} 
                               setchu=0;Setup_4 = Setup_4 + 1;giatriEncoder=Setup_4;
                             }    
                         }
                       nhanSetup=2;      
                   }
                 if(Setup_4==33){dongchop=0;choptat();if (chopchu>0){SetupLCD_3_4_1();}}
                 else SetupLCD_3_4_1(); 
         ...
         else if(Setup_1==3 && Setup_2==4 && Setup_3==2)//vao SETUP PORT
             {
                if(nhanSetup==0)  
                  {
                     if(giatriEncoder<=0){giatriEncoder=9999;}
                     else if(giatriEncoder>=10000){giatriEncoder=0;}
                     port=giatriEncoder; 
                  }
                else if(nhanSetup==1)
                   {
   ...
   Serial.print("Setup:");Serial.print(Setup_1);Serial.print(".");Serial.print(Setup_2);Serial.print(".");Serial.print(Setup_3);Serial.print(".");Serial.println(Setup_4);
  }
/////////////////////////////////////////////////////////////////////////
void SetupLCD_0()
    {
        ...
        if(Setup_1==1)
           { 
             lcd.setCursor(0,0);if(ngonngu==0)lcd.print("OUTPUT          ");else lcd.print("NGO RA          ");
             lcd.setCursor(0,1);if(ngonngu==0)lcd.print("PARAMETER       ");else lcd.print("THONG SO        ");     
           }
        if(Setup_1==2)
           { 
             if (dongchop==1)
               ...
               {
                 lcd.setCursor(0,0);if(ngonngu==0)lcd.print("PARAMETER       ");else lcd.print("THONG SO        ");
                 lcd.setCursor(0,1);if(ngonngu==0)lcd.print("SETUP           ");else lcd.print("CAI DAT         ");
                }   
              }
        if(Setup_1==3)
           { 
             if (dongchop==1)
               ...
               {
                 lcd.setCursor(0,0);if(ngonngu==0)lcd.print("SETUP           ");else lcd.print("CAI DAT         ");
                 lcd.setCursor(0,1);if(ngonngu==0)lcd.print("LANGUAGE        ");else lcd.print("NGON NGU        ");
                }   
              }
        if(Setup_1==4)
           { 
             if (dongchop==1)
               ...
               {
                 lcd.setCursor(0,0);if(ngonngu==0)lcd.print("LANGUAGE        ");else lcd.print("NGON NGU        ");
                 lcd.setCursor(0,1);if(ngonngu==0)lcd.print("EXIT            ");else lcd.print("THOAT           ");
                }   
            }
        if(Setup_1==5)
           { 
...
void SetupLCD_1()
    {
      if(Setup_2==1)
           { 
             lcd.setCursor(0,0);if(ngonngu==0)lcd.print("IN:  ");else lcd.print("NGVAO:");
      ...
      if(Setup_2==10||Setup_2==11)
           { 
             if(Setup_2==10)dongchop=0;else dongchop=1;
             choptat();
             if (chopchu>0)
               {
                lcd.setCursor(0,0);if(ngonngu==0)lcd.print("RESET ...       ");else lcd.print("CAI LAI ...     ");
                lcd.setCursor(0,1);if(ngonngu==0)lcd.print("EXIT            ");else lcd.print("THOAT           ");
               } 
           }
      else
          ...
          for(int i=2; i<=9;i++)
            {
              if(Setup_2 == i)
                 { 
                   lcd.setCursor(0,0);if(ngonngu==0){lcd.print("OUT");lcd.print(i-1);lcd.print(":");}else {lcd.print("NGO");lcd.print(i-1);lcd.print(":");}                 
        ...
        if(Setup_3==11)
          {
            ...
            if (chopchu>0)
               {
                  lcd.setCursor(0,0);if(ngonngu==0)lcd.print("EXIT            ");else lcd.print("THOAT           ");
                  lcd.setCursor(0,1);if(ngonngu==0)lcd.print("                ");else lcd.print("                ");
               }
          }
        else
          {
            ...
            if (chopchu>0)
               {
                  if(Setup_3==1)     {if(ngonngu==0)lcd.print("ALL             ");else lcd.print("TAT CA          ");}
                  else if(Setup_3==2){if(ngonngu==0)lcd.print("IN              ");else lcd.print("NGO VAO         ");}
                  else
                    {
                      if(ngonngu==0)sprintf(temp,"OUT %d           ",(Setup_3-2));else sprintf(temp,"NGO RA %d        ",(Setup_3-2));
                      lcd.print(temp);
                    }
               } 
          }
...
void SetupLCD_3()//SETUP
    {
      if(Setup_2==1)
        { 
          lcd.setCursor(0,0);if(ngonngu==0)lcd.print("NAME SETUP      ");else lcd.print("CAI DAT TEN     ");
          lcd.setCursor(0,1);if(ngonngu==0)lcd.print("ID SETUP        ");else lcd.print("CAI DAT MA      ");     
        }
      if(Setup_2==2)
        { 
          if (dongchop==1)
            ...
            {
              lcd.setCursor(0,0);if(ngonngu==0)lcd.print("ID SETUP        ");else lcd.print("CAI DAT MA      ");
              lcd.setCursor(0,1);if(ngonngu==0)lcd.print("IP SETUP        ");else lcd.print("CD DIA CHI      ");
            }   
         }
     if(Setup_2==3)
      { 
         if (dongchop==1)
            ...
            {
              lcd.setCursor(0,0);if(ngonngu==0)lcd.print("IP SETUP        ");else lcd.print("CAI DAT IP      ");
              lcd.setCursor(0,1);if(ngonngu==0)lcd.print("SERVER SETUP    ");else lcd.print("CD MAY CHU      ");
            }   
       }
     if(Setup_2==4)
      { 
         if (dongchop==1)
            ...
            {
              lcd.setCursor(0,0);if(ngonngu==0)lcd.print("SERVER SETUP    ");else lcd.print("CD MAY CHU      "); 
              lcd.setCursor(0,1);if(ngonngu==0)lcd.print("EXIT            ");else lcd.print("THOAT           ");
            }   
       }
     if(Setup_2==5)
...
void SetupLCD_3_1()//NAME SETUP
    {    
        lcd.setCursor(0,0);if(ngonngu==0)lcd.print("NAME:      ");else lcd.print("TEN:       ");
        for (int i=0;i<5;i++) 
          {  
            if(Setup_3<6)
              {    
                  ...
                  if((11+i) != (10+Setup_3))
                  ...
                  else
                    {
                      ...
                      if (now - timer_chopchu> 100) 
                         {
                           timer_chopchu = now;
                           chopchu++;if(chopchu==2)chopchu=0;
                           if(chopchu==0) {if(setchu==1)lcd.write(255);else lcd.print("_");}
                           else  lcd.print(chuChar[Name[i]]);         
                         }         
                    }   
              }  
            else{lcd.setCursor(11+i, 0);lcd.print(chuChar[Name[i]]);}
          }   
        lcd.setCursor(0,1);if(ngonngu==0)lcd.print("EXIT            ");else lcd.print("THOAT           ");  
...
void SetupLCD_3_2()//ID SETUP
    {    
        if(ngonngu==0)sprintf(temp,"ID:          %3d",IDtam);else sprintf(temp,"MA:          %3d",IDtam);
        lcd.setCursor(0,0);lcd.print(temp);
...
void SetupLCD_3_3()//IP SETUP
    {  
        if(Setup_3==1)//ip dong
          {
            lcd.setCursor(0,0);if(ngonngu==0)lcd.print("IP AUTOMATICALLY");lcd.print("IP TU DONG      ");
            lcd.setCursor(0,1);lcd.print("              ");
          }
        else if(Setup_3==2)//ip tinh
          {
            lcd.setCursor(0,0);if(ngonngu==0)lcd.print("IP MANUALLY     ");lcd.print("IP THU CONG     ");
            lcd.setCursor(0,1);lcd.print("              ");
          }
    }
...
void SetupLCD_3_3_2()
    { 
      ...
      if(Setup_4==1)//ipaddress[]
        { 
          lcd.setCursor(0,0);if(ngonngu==0)lcd.print("IP ADDRESS      ");else lcd.print("DIA CHI IP      ");
          if(setIPtinh==0)
            {
                ...
                if(setIPtinh == 5)
                  {
                    dongchop=1;
                    choptat();
                    if (chopchu>0){lcd.setCursor(0,1);if(ngonngu==0)lcd.print("EXIT            ");else lcd.print("THOAT           "); }
                  }
                else
                  {
                      for (int i=1;i<5;i++) 
                           ...
                           if(setIPtinh == i)
                               {
                                 long now = millis();
                                  ...
                                  if(chopchu==0) 
                                     { 
                                       if(setgiatriIP==256){lcd.print("___");}
                                       else{lcd.write(255);lcd.write(255);lcd.write(255);}
                                     }
                                  else
                                    {
                                      if(setgiatriIP==256){sprintf(temp,"%3d",ipaddress[i-1]);}
                                      else{sprintf(temp,"%3d",setgiatriIP);}
                                      lcd.print(temp);
                                    }
                               }                
                           else
        ...
        {  
          lcd.setCursor(0,0);if(ngonngu==0)lcd.print("DEFAULT GATEWAY ");else lcd.print("CONG MAC DINH   "); 
          if(setIPtinh==0)
            {
                           ...
                           if(setIPtinh == i)
                               {
                                 long now = millis();
                                  ...
                                  if(chopchu==0) 
                                    ...
                                    {
                                      if(setgiatriIP==256){sprintf(temp,"%3d",gateway[i-1]);}
                                      else{sprintf(temp,"%3d",setgiatriIP);}
                                      lcd.print(temp);
                                    }
                               }                
        ...
        { 
          ...
          lcd.setCursor(0,0);if(ngonngu==0)lcd.print("SUBNET MASK     ");else lcd.print("MAT NA MANG CON ");
          if(setIPtinh==0)
            {
                           ...
                           if(setIPtinh == i)
                               {
                                 long now = millis();
                                  ...
                                  if(chopchu==0) 
                                    ...
                                    {
                                      if(setgiatriIP==256){sprintf(temp,"%3d",subnet[i-1]);}
                                      else{sprintf(temp,"%3d",setgiatriIP);}
                                      lcd.print(temp);
                                    }
                               }                
...
void SetupLCD_3_4()//SERVER SETUP
    {  
        if(Setup_3==1)
          { 
            lcd.setCursor(0,0);if(ngonngu==0)lcd.print("SERVER ADDRESS  ");else lcd.print("DIA CHI MAY CHU ");
            lcd.setCursor(0,1);if(ngonngu==0)lcd.print("PORT            ");else lcd.print("CONG QD DU LIEU ");     
          }
        if(Setup_3==2)
          { 
            if (dongchop==1)
              ...
              {
                lcd.setCursor(0,0);if(ngonngu==0)lcd.print("PORT            ");else lcd.print("CONG QD DU LIEU ");
                lcd.setCursor(0,1);if(ngonngu==0)lcd.print("USER NAME       ");else lcd.print("TEN TAI KHOAN   ");
              }   
           }
       if(Setup_3==3)
        { 
           if (dongchop==1)
              ...
              {
                lcd.setCursor(0,0);if(ngonngu==0)lcd.print("USER NAME       ");else lcd.print("TEN TAI KHOAN   ");
                lcd.setCursor(0,1);if(ngonngu==0)lcd.print("PASSWORD        ");else lcd.print("MAT KHAU        ");
              }   
         }
       if(Setup_3==4)
         {
           if (dongchop==1)
              ...
              {
                lcd.setCursor(0,0);if(ngonngu==0)lcd.print("PASSWORD        ");else lcd.print("MAT KHAU        "); 
                lcd.setCursor(0,1);if(ngonngu==0)lcd.print("EXIT            ");else lcd.print("THOAT           ");
              }   
         }
       if(Setup_3==5)
                      ...
                      if (now - timer_chopchu> 100) 
                         {
                           ...
                           chopchu++;if(chopchu==2)chopchu=0;
                           if(chopchu==0) {if(setchu==1)lcd.write(255);else lcd.print("_");}
                           else  lcd.print(chuChar[ServerAddress[i]]);         
                         }         
                    }   
              }  
          }  
...
void SetupLCD_3_4_2()//PORT SETUP
    {
      if(ngonngu==0)
        ...
        {
          lcd.setCursor(0,0);lcd.print("CONG QD DU LIEU:");
...
void SetupLCD_4()
    {
        lcd.setCursor(0,0);if(ngonngu==0)lcd.print("ENGLISH         ");else lcd.print("TIENG ANH       ");
        lcd.setCursor(0,1);if(ngonngu==0)lcd.print("VIETNAMESE      ");else lcd.print("TIENG VIET      ");
    }
////////////////////////////////////////////////////////////////////////
void choptat()
  {
   ...
   if (now - timer_chopchu> 100) 
     {
       timer_chopchu = now;
       chopchu++;if(chopchu==2)chopchu=0;
       if(chopchu==0){lcd.setCursor(0,dongchop);lcd.print("                ");}            
     }
  }
////////////////////////////////////////////////////////////////////////
void docencoder()
  { 
      ...
      {  
        ...
        if(pulseIn(A, 1)>100)//thời gian xung kênh A từ mức cao xuống mức thấp > 50mls  
            { 
              ...
              chopchu=0;
              if(digitalRead(B)==1){giatriEncoder++;dongchop=1;}
              if(digitalRead(B)==0){giatriEncoder--;dongchop=0;}
          ...
          else
            {
               if (now - timer_thoatSetup > 20000)
                  {
                     timer_thoatSetup = now;
                     Setup_1=0 ;Setup_2=0;Setup_3=0; Setup_4=0;
                     lcd.clear();
                  }
            }
      } 
   ...
   if(nut==0 && nhanSetup==0 ){ nhanSetup=1;long now = millis();timer_thoatSetup = now;}//Serial.println("OK");
   else if(nut==1){nhanSetup=0;}//Serial.println("nha");
...
void docEEPROM()
 ...
 //////////////////////////////////////////////////////////////////////////////////////////
     ...
     for(int i=0;i<5;i++){Name[i]=EEPROM.read(101+i);Nametemp[i] = Name[i];}
     
     ID = EEPROM.read(106);
     
     IPtinh = EEPROM.read(120); 
     
     for(int i=0;i<4;i++){ipaddress[i]=EEPROM.read(121+i);}
     for(int i=0;i<4;i++){gateway[i]=EEPROM.read(125+i);}
     for(int i=0;i<4;i++){subnet[i]=EEPROM.read(129+i);}

     for(int i=0;i<32;i++){ServerAddress[i]=EEPROM.read(141+i);ServerAddresstemp[i] = ServerAddress[i];}
...
void loop()
  { 
    if(ketnoimang==0){ConnectEthernet();}
    else
      { 
        if (!mqttClient.connected()){ketnoiserver=0; reconnectserver();}
        ethernet();  
      }
    if(lenh==1)
      { 
        long now = millis();timer_ketnoiserver = now+timerketnoiserver;
        //if(khoidong==0 )dieukhienrelay();//&& khongcheAuto==0
        dieukhienrelay();
        lenh=0;khoidong=0;
      }
    doctrangthai();
    if(ketnoiserver==1)guitrangthai();
