...
char mqtt_server[48] = "mqtt.dev.altasoftware.vn";
int mqtt_port = 1883;
char mqtt_user[32] = "altamedia";
char mqtt_password[32] = "Altamedia@%";
// char *mqtt_client_id = "DEMO_CONTROL_POWER_2025";
// CONTROL_POWER + MAC_ID
char mqtt_client_id[40]; // đủ dài cho perfix + 12 kí tự MAC
...
{
  // đang cấu hình mạng
  Serial.print(F("[NET] Đang cấu hình IP: "));
  Serial.println(ip);
  Serial.println(F("  (vui lòng đợi...)"));

  // Nếu muốn đo luôn thời gian Ethernet.begin():
  unsigned long t0 = millis();

  Ethernet.begin(mac, ip, dns, gateway, subnet);

  // unsigned long dt = millis() - t0;
  // Serial.print(F("[NET] Applied manual configuration ("));
  // Serial.print(dt);
  // Serial.println(F(" ms)"));
  ...
  // Cập nhật MQTT Client ID theo MAC
  build_mqtt_client_id();

  // Serial.print(F("  IP thực tế  : "));
...
// Sinh MQTT client ID = CONTROL+ POWER + MAC ethernet
void build_mqtt_client_id()
{
  // 17 ký tự: 6 byte * 2 hex + 5 dấu ":" + null
  char macStr[18];

  // MAC dạng DE:AD:BE:EF:12:34
  ...
  // CONTROL_POWER-DE:AD:BE:EF:12:34
  snprintf(mqtt_client_id, sizeof(mqtt_client_id),
           "CONTROL_POWER-%s", macStr);
...
// lưu mac
const uint8_t EEPROM_MAC_SIGNATURE = 0x4D; // 'M' – đánh dấu đã có MAC
const int EEPROM_MAC_SIGNATURE_ADDR = EEPROM_NET_MQTT_PORT_ADDR + sizeof(mqtt_port);
const int EEPROM_MAC_ADDR = EEPROM_MAC_SIGNATURE_ADDR + 1; // 6 byte: MAC[0..5]
// //----------------
...
static void clearRetainedCmd()
{
  ...
  mqttClient.publish(topic_cmd_sub, (const uint8_t *)"", 0, false);
}
...
void setup()
{
  Serial.begin(9600);
  delay(100);
  loadOrGenerateMac(); // load địa chỉ mac
  lcdv2_begin();
...
// Ghi/đọc MAC vào EEPROM
static void writeMacToEEPROM(int addr, const byte mac[6])
{
...
static void readMacFromEEPROM(int addr, byte mac[6])
{
  for (uint8_t i = 0; i < 6; ++i)
  {
...
// Sinh MAC ngẫu nhiên: local, unicast (bit 0 = 0, bit 1 = 1)
static void generateRandomMac(byte mac[6])
{
  mac[0] = 0x02; // 00000010b: local admin + unicast
  for (uint8_t i = 1; i < 6; ++i)
  {
    mac[i] = (byte)random(0, 256);
  }
}

// Load MAC từ EEPROM, nếu chưa có thì tự sinh rồi lưu lại
void loadOrGenerateMac()
{
  uint8_t sig = EEPROM.read(EEPROM_MAC_SIGNATURE_ADDR);
  if (sig == EEPROM_MAC_SIGNATURE)
  {
    // Đã có MAC trong EEPROM
    readMacFromEEPROM(EEPROM_MAC_ADDR, mac);
  }
  else
  {
    // Chưa có → seed random, sinh MAC mới rồi lưu lại
    randomSeed(analogRead(A0) + ((unsigned long)analogRead(A1) << 10) + micros());

    generateRandomMac(mac);
    writeMacToEEPROM(EEPROM_MAC_ADDR, mac);
    EEPROM.update(EEPROM_MAC_SIGNATURE_ADDR, EEPROM_MAC_SIGNATURE);
  }
